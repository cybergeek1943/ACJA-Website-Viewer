...
func button_arrange(buttons: Array, location: Array):
	...
	for loc in location:
		get_node('buttons/' + buttons[button_count] + 'button_anim_loc').play(loc)
		button_count += 1
...
func _arrange_buttons_initiated():
	...
	for button in ['browser', 'encyclopedia', 'khan_academy', 'typing', 'fun_educational_websites']:
		if not Global.user_data['hw'][button]:
			...
			get_node('buttons/' + button).show()
		else:
			get_node('buttons/' + button).hide()
	...
	if Global.user_data['hw']['national_geographic']:
		$buttons/fun_brain/button_anim_loc.play("center")
		$buttons/national_geographic.hide()
	elif Global.user_data['hw']['fun_brain']:
		$buttons/national_geographic/button_anim_loc.play('center')
		$buttons/fun_brain.hide()
	else:
		$buttons/national_geographic/button_anim_loc.play('left')
		$buttons/fun_brain/button_anim_loc.play('right')
		$buttons/national_geographic.show()
		$buttons/fun_brain.show()
...
func _on_fun_educational_websites_pressed():
	$buttons/page_change.play('fun_educational_websites')
	$back/AnimationPlayer.play('back')
func _on_back_pressed():
	$buttons/page_change.play_backwards('fun_educational_websites')
